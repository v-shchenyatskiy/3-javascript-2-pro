...
const productFile = path.resolve(__dirname, './db/0_products.json');
const cartFile = path.resolve(__dirname, './db/1_cart.json');

const productFileNew = path.resolve(__dirname, './db/3_catalog_new.json');
...
app.get('/api/catalog-new', (req, res) => {
  fs.readFile(productFileNew, 'utf-8', (err, data) => {
    if (err) {
      res.send(JSON.stringify({result: 0, text: err}));
    } else {
      res.send(data);
